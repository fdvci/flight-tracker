...
1. Serve the directory over HTTP (required so the OpenSky API fetch works reliably and the module imports load). Any static server works:
   ```bash
   python -m http.server 4173
   # then open http://localhost:4173
   ```
2. The app auto-fetches flight states every 20 seconds and renders them as instanced neon aircraft on the globe. It will first attempt the direct OpenSky endpoint and transparently fall back to CORS relays if the browser blocks the request.
